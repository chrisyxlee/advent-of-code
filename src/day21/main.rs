...
fn main() {
    // --snip--
    let file_path = "tmp/day21/input.txt";
    println!("In file {}", file_path);

    {
        let (mut known, mut unknown) = parse_input_pt1(&read_lines(file_path));
        solve_pt1(&mut known, &mut unknown);
        println!("Part 1: {}", known[&String::from("root")]);
    }
    {
        let (mut known, mut unknown) = parse_input_pt2(&read_lines(file_path));
