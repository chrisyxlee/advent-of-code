...
impl Tok {
    ...
    pub fn get_value(&self, old: i32) -> i32 {
...
impl ThrowCondition {
    ...
    pub fn get_destination(&self, val: i32) -> i32 {
...
impl Monkey {
    ...
    fn perform_action(&mut self) -> Option<(i32, i32)> {
        if self.items.len() == 0 {
            return None;
        }
        let item = self.items.pop().expect("just checked array length");
        let new_val = self.calc.calculate(item) / 3;
        self.inspection_count += 1;
        return Some((new_val, self.divisor.get_destination(new_val)));
    }
